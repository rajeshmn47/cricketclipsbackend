...
// Get squad by seriesId and teamId
router.get("/allsquads", async (req, res) => {
    try {
        const { seriesId } = req.query;
        if (!seriesId) {
            return res.status(400).json({ message: "seriesId is required" });
        }
        const squads = await Squad.find({ seriesId });
...
// GET /api/match/lineup/:matchId
router.get("/lineup/:matchId", async (req, res) => {
    try {
        const { matchId } = req.params;
        ...
        const homeTeamObj = {
            ...
            currentKeeper: null,
            currentKeeperOverride: !!match.teamHomeKeeperPlayerId
        };

        // Process away team
        const awayPlayers = cleanPlayers(match.teamAwayPlayers);
        const awayTeamObj = {
            teamId: match.teamAwayId,
            teamName: match.teamAwayName || match.teamAwayId,
            players: awayPlayers,
            currentKeeperId: match.teamAwayKeeperPlayerId || null,
            currentKeeper: null,
        ...
        res.status(200).json({
            ...
            lineup: [homeTeamObj, awayTeamObj]
        });

    } catch (err) {
        console.error("Error fetching lineup:", err);
        res.status(500).json({ message: "Failed to fetch lineup", error: err.message });
    }
});

// PUT /api/match/keeper-override
router.put("/keeper-override", async (req, res) => {
    try {
        const { matchId, teamId, keeperPlayerId } = req.body;
        if (!matchId || !teamId || !keeperPlayerId) {
            return res.status(400).json({ message: "Missing required fields" });
        }

        const match = await MatchLiveDetails.findOne({ matchId: String(matchId) });
        if (!match) {
            return res.status(404).json({ message: "Match not found" });
        }

        // Determine which team's keeper to update
        let updateField;
        if (match.teamHomeId === teamId) {
            updateField = { teamHomeKeeperPlayerId: keeperPlayerId };
        } else if (match.teamAwayId === teamId) {
            updateField = { teamAwayKeeperPlayerId: keeperPlayerId };
        } else {
            return res.status(400).json({ message: "Team ID does not match home or away team of this match" });
        }

        await MatchLiveDetails.updateOne({ matchId: String(matchId) }, { $set: updateField });

        res.status(200).json({ message: "Override saved successfully" });
    } catch (err) {
        console.error("Error saving keeper override:", err);
