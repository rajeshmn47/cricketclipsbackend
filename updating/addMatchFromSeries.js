...
async function getMatchesFromSeries(seriesId) {
    let key = await matchkeys(seriesId);
    console.log(key, 'leys')
    const options = {
        method: "GET",
        url: `https://cricbuzz-cricket.p.rapidapi.com/series/v1/${seriesId}`,
        headers: {
            "x-rapidapi-key": "35e9efa6bbmshb28bf330d321713p1677f9jsn2b82cbc1bd1d",
...
async function getCurrentSeries() {
    ...
    const tenDaysLater = new Date(date.getTime() - 4 * 24 * 60 * 60 * 1000); // 10 days ahead
    ...
    const ongoingSeries = await Series.find({
        $or: [
            ...
            { endDate: { $lte: now, $gt: tenDaysLater } }
        ]
...
module.exports.addMatchesForAllCurrentSeries = async function () {
    ...
    for (const series of currentSeriesArr) {
        ...
        try {
            ...
            const matches=await Match.findOne({ seriesId: seriesId });
            if(matches?.length>0)
                continue
